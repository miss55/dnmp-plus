...
echo "Container package url     : ${ALPINE_REPOSITORIES}"
...
if [[ -z "${EXTENSIONS##*,gd,*}" ]]; then
    ...
    && docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/  --enable-gd --with-freetype --with-jpeg --with-webp \
...
if [[ -z "${EXTENSIONS##*,apcu,*}" ]]; then
    echo "---------- Install apcu ----------"
    installExtensionFromTgz pcu-5.1.21
fi
...
if [[ -z "${EXTENSIONS##*,mongodb,*}" ]]; then
    ...
    installExtensionFromTgz mongodb-1.13.0
fi
